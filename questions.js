...
let runQuiz = () => {
  ...
  for (let i = 0; i < currQuizQuestion.choices.length; i++) {
    ...
    if (
      currQuizQuestion.rightAnswer === i ||
      currQuizQuestion.rightAnswer === "all"
    ) {
...
let addTransition = () => {
    currQuizQuestion.transition ? renderTransitionScreen() : ''
}
...
next.addEventListener("click", event => {
    ...
    if (currQuizQuestion.wrongAnswer === "none") {
      ...
      if (
        selectedAnswers.includes(currQuizQuestion.wrongAnswer) ||
...
let renderTransitionScreen = () => {
  const transitionScreenHTML = `
    <section id= "transition-screen" class='animated fadeInDown'>
        ...
        ${currQuizQuestion.transition.transitionParagraph}
        </div>
    </section>
    `;
  //Clear the screen (clearScreen)
  ...
  container.innerHTML = transitionScreenHTML;
